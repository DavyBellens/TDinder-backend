...
const getAllMatches = async () => {
  try {
    ...
    console.log(error);
    throw new Error(
      "Database error when creating match. See server log for details."
    );
  }
};
...
const getMatchByProfiles = async (profileId1: number, profileId2: number) => {
  try {
    ...
    console.log(error);
    throw new Error(
      "Database error when getting match. See server log for details."
    );
  }
};
...
const deleteMatchById = async (id: number) => {
  try {
    ...
    console.log(error);
    throw new Error(
      "Database error when deleting match. See server log for details."
    );
  }
};
...
const getMatchesFromProfile = async (profileId: number) => {
  try {
    ...
    return matchesPrisma ? matchesPrisma.map((m) => Match.from(m)) : [];
  } catch (error) {
    console.log(error);
    throw new Error(
      "Database error when getting match. See server log for details."
