...
const createSwipe = async (
  ...
  if (direction === "R") {
    if (await swipeDb.getSwipeByIds(id, swipeeId))
      throw new Error("You already swiped this person");
    const isMatch = await swipeDb.getSwipeByIds(swipeeId, id);
    if (isMatch) {
      await swipeDb.createSwipe(id, swipeeId, direction);
      return {
        object: await matchService.match(id, swipeeId, auth),
        isMatch: true,
      };
    } else {
      return {
        object: await swipeDb.createSwipe(id, swipeeId, direction),
        isMatch: false,
      };
    }
  } else {
