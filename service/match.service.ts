...
const match = async (
  ...
  if (profileId1 == profileId2)
    ...
    const existingMatch = await matchDb.getMatchByProfiles(
      profile1.id,
      profile2.id
    );
    if (existingMatch)
      throw new Error(
        `profiles with id's ${profile1.id} ad ${profile2.id} already have a match`
      );
...
const unmatch = async (
  ...
  if (parseInt(cupid.id) != profileId1)
    throw new Error("Why the fuck you tryna interfere with love?");
  if (profileId1 == profileId2)
    throw new Error("Did you really just try to unmatch yourself?");
  else {
    const profile1 = await profileService.getProfileById(profileId1);
    const profile2 = await profileService.getProfileById(profileId2);
    const matchToDelete = await matchDb.getMatchByProfiles(
      profile1.id,
      profile2.id
    );
    if (!matchToDelete)
      throw new Error(
        `Profiles with id's ${profile1.id} and ${profile2.id} have no match`
      );
